<?php
    ...
    class DeleteManufacturersProduct
    {
        /**
         * @param $request
         * @param Closure $next
        ...
        public function handle($request, Closure $next)
        {
            try {
                ...
                $this->check_exists($data['manufacturers_products_id']);
                return $next($request);
            } catch (Exception $e) {
        ...
        private function check_exists($id){
            $data = DB::table('manufacturers_products')->where('id','=',$id)->exists();
            if(empty($data)){
        ...
        private function check_input_data($data){
            $array_required = ['manufacturers_products_id','token'];
